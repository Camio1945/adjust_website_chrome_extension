/** 记录哪些网站的定时任务已经开启，防止重复开启 */
let startedIntervals = {};

/** 每隔250毫秒执行页面的调整操作（不用担心长时间消耗CPU，后续代码会在10秒后停止执行interval） */
 ...
 * @param {number} intervalMs 持久化执行的间隔（毫秒），为空则不持久化执行（但会被主循环调用）
 * @param {function} logic 具体的调整逻辑
 */
function handleAdjustment(name, intervalMs, logic) {
    if (intervalMs) {
        if (startedIntervals[name]) return;
        startedIntervals[name] = true;
        logic(); // 立即执行一次
...
/** 调整Hutool */
function adjustHutool(intervalMs) {
...
/** 调整语言学习网站 */
function adjustLanguageReactor(intervalMs) {
    // 如果是翱象主档商品管理页面，才处理
    if (isHrefContainAnyStrInArr(["https://www.languagereactor.com/video-file"])) {
        handleAdjustment("adjustLanguageReactor", intervalMs, () => {
            if (document.getElementById("adjust-btn-lr")) return;

            // 1. Create the button
            const btn = document.createElement('button');
            btn.id = "adjust-btn-lr";
...
/** 调整美团闪购 */
function adjustTsccMeituan(intervalMs) {
    // 如果是美团闪购页面，才处理
    if (isHrefContainAnyStrInArr(["https://tscc.meituan.com/"])) {
        handleAdjustment("adjustTsccMeituan", intervalMs, () => {
            if (document.getElementById("adjust-btn-meituan")) return;

            // 1. Create the button
            const btn = document.createElement('button');
            btn.id = "adjust-btn-meituan";
...
/** -------------------------- 调整processon 开始 -------------------------- */
function adjustProcesson(intervalMs) {
    // 如果是processon页面，才处理
...
/** -------------------------- 调整酷壳 开始 -------------------------- */
function adjustCoolshell(intervalMs) {
    // 如果是酷壳页面，才处理
...
/** -------------------------- 调整cnblogs文章详情页面 开始 -------------------------- */
function adjustCnblogsArticle(intervalMs) {
...
/** -------------------------- 调整知乎 开始 -------------------------- */
function adjustZhiHu(intervalMs) {
    // 知乎通用页面处理
    if (isHrefContainAnyStrInArr(["https://www.zhihu.com"])) {
        handleAdjustment("adjustZhiHu", intervalMs, () => {
            // 自动关闭登录框
...
function adjustZhiHuAnswer(intervalMs) {
    ...
    if (isHrefContainAllStrInArr(["https://www.zhihu.com/question", "/answer/"])) {
        handleAdjustment("adjustZhiHuAnswer", intervalMs, () => {
            ...
            // 先判断元素是否存在，避免抛出异常阻断后面的代码
            const rewardEl = document.getElementsByClassName("Reward")[0];
            if (rewardEl?.parentNode) {
                rewardEl.parentNode.style.display = "none";
            }
            // 标题下面的邀请行（getElementsByClassName 不接受 "." 前缀，原写法永远匹配不到，
            // 改用 querySelector 并做存在性判断）
            const questionStatusEl = document.querySelector(".QuestionStatus-notification-inner");
            if (questionStatusEl?.parentNode?.parentNode?.parentNode) {
                questionStatusEl.parentNode.parentNode.parentNode.style.display = "none";
            }
            // 以下几行用于删除盐选答案
            ...
            // 以下几行用于删除由知乎直答提供的内容
            let element = getElementByFullText("span", "由知乎直答提供");
            if (element) {
...
/** -------------------------- 调整简书文章详情页面 开始 -------------------------- */
function adjustJianShuArticle(intervalMs) {
    // 如果是简书文章详情页面，才处理
...
function specialHandleOfJianShuArticle(intervalMs) {
    handleAdjustment("specialHandleOfJianShuArticle", intervalMs, () => {
...
/** -------------------------- 调整csdn博客的文章页面 开始 -------------------------- */
function adjustCsdnArticle(intervalMs) {
...
/** 根据全部文本内容获取元素（返回第一个符合条件的元素，而不是数组） */
function getElementByFullText(tag, text) {
    return Array.from(document.querySelectorAll(tag))
        .find(el => el.textContent.trim() === text);
}

/** 根据部分文本内容获取元素（返回第一个符合条件的元素，而不是数组） */
